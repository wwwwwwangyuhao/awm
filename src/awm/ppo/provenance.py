"""Immutable provenance manifest for formal PPO runs."""
...
def build_run_manifest(
    *,
    ...
    protocol_path = root / "configs" / "ppo_baseline_v1.json"
    ...
    return {
        "manifest_id": "awm-ppo-run-manifest-v1",
        "ppo_protocol_id": str(protocol["ppo_protocol_id"]),
        ...
        "ppo_protocol_sha256": _sha256_file(protocol_path),
...
def ensure_run_manifest(path: str | Path, expected: dict[str, Any]) -> Path:
    ...
    if destination.exists():
        ...
        if current != expected:
            ...
            raise RuntimeError(
                "PPO run manifest mismatch; refusing to mix provenance in one output directory: "
