"""Prepare raw primal-dual direct-MC directional variants on frozen v9 nodes.

Diagnostic only. Replays the formal batch exactly and uses the unstandardized
primal-dual advantage A_water - lambda_eta * A_risk^MC.  Tail-Q and component
z-scoring are absent.  The same numerically-derived Fisher damping and exact-KL
perturbation used by the prior directional diagnostics are retained so only the
actor objective scaling changes.
"""
...
def prepare(
    *, root: Path, checkpoint: Path, metrics: Path, rollout_update: int,
    ...
    # Direct complete-episode Monte-Carlo tail-risk advantage. With the frozen
    # gamma=lambda=1 rollout contract this is h_i - V_r(s_t).  For this
    # diagnostic the actor uses the *raw* primal-dual Lagrangian advantage; no
    # component z-scoring is applied.
    risk_credit = batch.risk_advantages.to(agent.device)
    standardized_adv, standardized_diag = agent._condition_actor_advantages(
        reward_adv=reward_adv, risk_adv=risk_credit, etas=etas,
    )
    lambda_per_transition = agent._lambda_tensor(etas)
    combined_adv = reward_adv - lambda_per_transition * risk_credit
    ...
    surrogate = (ratio * combined_adv.detach()).mean()
    grads = torch.autograd.grad(surrogate, params, allow_unused=True, retain_graph=True)
    g = _flat_tensors(list(grads), params).detach()
    standardized_surrogate = (ratio * standardized_adv.detach()).mean()
    standardized_grads = torch.autograd.grad(
        standardized_surrogate, params, allow_unused=True
    )
    g_standardized = _flat_tensors(list(standardized_grads), params).detach()
    gradient_cosine_raw_vs_standardized = float(
        torch.dot(g, g_standardized).item()
        / max(
            1e-30,
            float(torch.linalg.vector_norm(g).item())
            * float(torch.linalg.vector_norm(g_standardized).item()),
        )
    )
    ...
    manifest = {
        "diagnostic_id": "awm-rcwa-v9-raw-mc-primal-dual-directional-prepare-v1",
        "risk_credit_source": "direct_mc_episode_tail_advantage_raw_primal_dual",
        ...
        "standardized_policy_gradient_norm": float(torch.linalg.vector_norm(g_standardized).item()),
        "raw_vs_standardized_gradient_cosine": gradient_cosine_raw_vs_standardized,
        "raw_combined_advantage_mean": float(combined_adv.mean().item()),
        "raw_combined_advantage_std": float(combined_adv.std(unbiased=False).item()),
        "mc_risk_credit_mean": float(risk_credit.mean().item()),
        "mc_risk_credit_std": float(risk_credit.std(unbiased=False).item()),
        "standardized_advantage_diagnostics": standardized_diag,
    }
...
def main() -> None:
    p = argparse.ArgumentParser(description="Prepare raw direct-MC primal-dual directional variants with damped Fisher")
