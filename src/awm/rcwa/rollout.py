...
class RCWAEpisodeOutcome:
    ...
    dssat_ircm_mm: float
    yield_kg_ha: float
    signals: RCWAEpisodeBreakdown


@dataclass(frozen=True, slots=True)
class BalancedRCWARolloutResult:
    batch: RCWARolloutBatch
    outcomes: tuple[RCWAEpisodeOutcome, ...]
...
def collect_episode(
    ...
    *,
    ...
    while True:
        ...
        action, reward_value, risk_value = agent.act(state_tensor)
        ...
        buffer.append(
            ...
            risk_value=float(risk_value.item()),
            reward=water_reward,
    ...
    buffer.register_terminal_retention(
        eta=float(cell.eta),
        retention=float(breakdown.yield_retention),
    )
...
def collect_balanced_training_rollout(
    *,
    ...
    if expected_size != 6750:
        raise AssertionError("formal RCWA rollout must contain 6750 transitions")
    buffer = RCWARolloutBuffer(
    ...
    return BalancedRCWARolloutResult(
        batch=batch,
