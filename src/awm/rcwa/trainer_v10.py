"""Sequential, resumable real-DSSAT trainer for RCWA-RL v10."""
...
def hyperparameters_from_protocol(protocol: dict[str, Any]) -> RCWAHyperparameters:
    ...
    risk = protocol["risk_constraint"]
    lagrangian = protocol["lagrangian"]
    environment = protocol["environment"]
    return RCWAHyperparameters(
        state_dim=int(environment["observation_dim"]),
        actor_hidden_dims=tuple(int(x) for x in policy["actor_hidden_dims"]),
        reward_critic_hidden_dims=tuple(int(x) for x in policy["reward_critic_hidden_dims"]),
        risk_critic_hidden_dims=tuple(int(x) for x in policy["risk_critic_hidden_dims"]),
        learning_rate=float(optimizer["learning_rate"]),
        gamma=float(optimizer["gamma"]),
        gae_lambda=float(optimizer["gae_lambda"]),
        risk_gamma=float(optimizer["risk_gamma"]),
        risk_gae_lambda=float(optimizer["risk_gae_lambda"]),
        ...
        risk_value_loss_coefficient=float(optimizer["risk_value_loss_coefficient"]),
        ...
        dual_initial_value=float(lagrangian["dual_initial_value"]),
        dual_learning_rate=float(lagrangian["dual_learning_rate"]),
    )


class RCWAV10Trainer:
    def __init__(
        self,
        *,
        ...
        self.agent = RCWAV10Agent(hyperparameters=self.hparams, seed=self.seed, device=device)
        self.normalizer = RunningObservationNormalizer(state_dim=self.hparams.state_dim)
        self.output_dir = (
            Path(output_dir).expanduser().resolve()
            if output_dir is not None
            else (self.root / "runtime" / "rcwa_rl_v10" / f"seed_{self.seed}").resolve()
        )
        try:
            self.output_dir.relative_to(self.root)
        except ValueError as exc:
            raise ValueError("RCWA output_dir must remain inside the AWM project root") from exc
    ...
    def run_update(self) -> dict[str, Any]:
        ...
        if stats.update_index != update_number:
            raise RuntimeError("RCWA update index drifted from rollout scheduler index")
        outcomes = rollout.outcomes
        payload = {
            "status": "passed",
            "rcwa_protocol_id": self.protocol["rcwa_protocol_id"],
            "seed": self.seed,
            ...
            "eta_summary": {
                f"{eta:.2f}": {
                    ...
                    "mean_retention": mean(
                        x.signals.yield_retention for x in outcomes if abs(x.eta - eta) <= 1e-12
                    ),
                    "tau": stats.tau_by_eta[f"{eta:.2f}"],
                    "lcvar": stats.lcvar_by_eta[f"{eta:.2f}"],
                    "violation": stats.violation_by_eta[f"{eta:.2f}"],
    ...
    def _checkpoint_payload(self) -> dict[str, object]:
        ...
        return {
            "trainer_protocol_id": "awm-rcwa-trainer-v10",
    ...
    def checkpoint_path(self, update_index: int | None = None) -> Path:
        ...
        return self.recovery_checkpoint_dir / f"rcwa_seed{self.seed}_update{idx:04d}.pt"

    def candidate_checkpoint_path(self, update_index: int | None = None) -> Path:
        idx = self.agent.update_index if update_index is None else int(update_index)
        return self.candidate_checkpoint_dir / f"rcwa_seed{self.seed}_update{idx:04d}.pt"
    ...
    def load_checkpoint(self, path: str | Path) -> None:
        ...
        if payload.get("trainer_protocol_id") != "awm-rcwa-trainer-v10":
    ...
    def validate_current_checkpoint(self) -> dict[str, Any]:
        checkpoint_id = f"rcwa_seed{self.seed}_update{self.agent.update_index:04d}"
    ...
    def train_until(self, stop_after_update: int) -> dict[str, Any]:
        ...
        if candidate_interval != validation_interval:
            raise RuntimeError("candidate and validation cadence must match in RCWA Protocol v10")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train AWM RCWA-RL v10 on real DSSAT")
...
__all__ = ["RCWAV10Trainer", "hyperparameters_from_protocol"]
