"""Prepare direct-MC tail-risk directional variants on frozen v9 nodes.

Diagnostic only. Replays the formal batch exactly and replaces only the
Tail-Q actor risk credit with the rollout's direct Monte-Carlo risk advantage.
The same numerically-derived Fisher damping and exact-KL perturbation are used.
"""
...
def prepare(
    *, root: Path, checkpoint: Path, metrics: Path, rollout_update: int,
    ...
    # gamma=lambda=1 rollout contract this is h_i - V_r(s_t), where h_i is the
    # terminal tail cost repeated across the episode. No Tail-Q fit is used.
    ...
    manifest = {
        "diagnostic_id": "awm-rcwa-v9-mc-tail-directional-prepare-v1",
        "risk_credit_source": "direct_mc_episode_tail_advantage",
...
def main() -> None:
    p = argparse.ArgumentParser(description="Prepare direct-MC tail-risk directional variants with damped Fisher")
