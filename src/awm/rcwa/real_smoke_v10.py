"""One untrained RCWA-RL v10 episode through the formal real-DSSAT stack."""
...
def run_real_rcwa_v10_smoke(
    ...
    *,
    ...
    if config.get("rcwa_protocol_id") != "awm-rcwa-rl-v10":
    ...
    agent = RCWAV10Agent(seed=seed, device=str(config.get("device", "cpu")))
    ...
    factory = PPORealEnvFactory(
        project_root=root,
        work_dir=root / "runtime" / "rcwa_rl_v10" / "single_episode_smoke",
    ...
    result = {
        "status": "passed",
        "rcwa_protocol_id": "awm-rcwa-rl-v10",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one untrained RCWA-RL v10 episode on real DSSAT")
    parser.add_argument("config")
    parser.add_argument("--project-root", default=".")
    parser.add_argument("--runtime-base")
    parser.add_argument("--audit-output")
    args = parser.parse_args()
    result = run_real_rcwa_v10_smoke(
        args.config,
...
__all__ = ["run_real_rcwa_v10_smoke"]
