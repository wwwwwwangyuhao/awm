"""Sequential, resumable real-DSSAT trainer for PPO-Lagrangian baseline v1."""
...
PROTOCOL_PATH = "configs/ppo_lagrangian_baseline_v1.json"
...
def hyperparameters_from_protocol(protocol: dict[str, Any]) -> PPOLagrangianHyperparameters:
    policy = protocol["policy"]
    critics = protocol["critics"]
    actor_update = protocol["actor_update"]
    optimizer = protocol["optimizer"]
    lagrangian = protocol["lagrangian"]
    return PPOLagrangianHyperparameters(
        state_dim=int(policy["observation_dim"]),
        actor_hidden_dims=tuple(int(x) for x in policy["actor_hidden_dims"]),
        reward_critic_hidden_dims=tuple(int(x) for x in critics["reward_critic_hidden_dims"]),
        cost_critic_hidden_dims=tuple(int(x) for x in critics["cost_critic_hidden_dims"]),
        learning_rate=float(optimizer["learning_rate"]),
        gamma=float(critics["reward_gamma"]),
        gae_lambda=float(critics["reward_gae_lambda"]),
        cost_gamma=float(critics["cost_gamma"]),
        cost_gae_lambda=float(critics["cost_gae_lambda"]),
        clip_epsilon=float(actor_update["ppo_clip_epsilon"]),
        update_epochs=int(actor_update["update_epochs"]),
        minibatch_size=int(actor_update["minibatch_size"]),
        reward_value_loss_coefficient=float(optimizer["reward_value_loss_coefficient"]),
        cost_value_loss_coefficient=float(optimizer["cost_value_loss_coefficient"]),
        entropy_coefficient=float(actor_update["entropy_coefficient"]),
        max_grad_norm=float(actor_update["max_grad_norm"]),
        adam_beta1=float(optimizer["adam_beta1"]),
        adam_beta2=float(optimizer["adam_beta2"]),
        adam_eps=float(optimizer["adam_eps"]),
        dual_initial_value=float(lagrangian["initial_value"]),
        dual_learning_rate=float(lagrangian["dual_learning_rate"]),
    )
...
class PPOLagrangianTrainer:
    def __init__(
        ...
        *,
        ...
        self.output_dir = (
            ...
            else (self.root / "runtime" / "ppo_lagrangian_v1" / f"seed_{self.seed}").resolve()
        )
        try:
            self.output_dir.relative_to(self.root)
        except ValueError as exc:
            raise ValueError("PPO-Lagrangian output_dir must remain inside AWM project root") from exc
        ...
        self.run_manifest_path = ensure_run_manifest(
            self.output_dir / "run_manifest.json", self.run_manifest
        )
        ...
        for path in (
            self.validation_dir,
            self.recovery_checkpoint_dir,
            self.candidate_checkpoint_dir,
        ):
    ...
    def run_update(self) -> dict[str, Any]:
        ...
        if stats.update_index != update_number:
            raise RuntimeError("update index drifted from balanced rollout scheduler")
        outcomes = rollout.outcomes
        eta_summary: dict[str, dict[str, float | int | bool]] = {}
        for eta in (0.90, 0.95, 0.98):
            group = [x for x in outcomes if abs(x.eta - eta) <= 1e-12]
            if len(group) != 18:
                raise RuntimeError(f"expected 18 training episodes for eta={eta:.2f}")
            mean_retention = mean(x.signals.yield_retention for x in group)
            mean_cost = mean(x.signals.signed_constraint_cost for x in group)
            eta_summary[f"{eta:.2f}"] = {
                "episode_count": len(group),
                "mean_policy_irrigation_mm": mean(x.policy_irrigation_mm for x in group),
                "mean_retention": mean_retention,
                "mean_signed_constraint_cost": mean_cost,
                "expected_constraint_satisfied": mean_cost <= 1e-12,
                "dual_before": stats.dual_before[f"{eta:.2f}"],
                "dual_after": stats.dual_after[f"{eta:.2f}"],
            }
        payload = {
            "status": "passed",
            ...
            "mean_water_return": mean(x.signals.water_return for x in outcomes),
    ...
    def _checkpoint_payload(self) -> dict[str, object]:
        return {
            "trainer_protocol_id": "awm-ppo-lagrangian-trainer-v1",
            "run_manifest": self.run_manifest,
            "agent": self.agent.checkpoint_payload(),
            "normalizer": asdict(self.normalizer.state()),
            "transition_count": self.transition_count,
        }
    ...
    def checkpoint_path(self, update_index: int | None = None) -> Path:
        ...
        return self.recovery_checkpoint_dir / f"ppo_lag_seed{self.seed}_update{idx:04d}.pt"

    def candidate_checkpoint_path(self, update_index: int | None = None) -> Path:
        idx = self.agent.update_index if update_index is None else int(update_index)
        return self.candidate_checkpoint_dir / f"ppo_lag_seed{self.seed}_update{idx:04d}.pt"
    ...
    def save_candidate_checkpoint(self) -> Path:
        ...
        if self.agent.update_index <= 0 or self.agent.update_index % interval != 0:
            raise ValueError(
                f"update {self.agent.update_index} is not on candidate cadence {interval}"
            )
    ...
    def load_checkpoint(self, path: str | Path) -> None:
        ...
        if payload.get("trainer_protocol_id") != "awm-ppo-lagrangian-trainer-v1":
            raise ValueError("trainer checkpoint protocol mismatch")
        if payload.get("run_manifest") != self.run_manifest:
            raise ValueError("checkpoint run manifest does not match current provenance")
        ...
        if int(payload["transition_count"]) != self.transition_count:
            raise ValueError("checkpoint transition_count inconsistent with update index")

    def validate_current_checkpoint(self) -> dict[str, Any]:
        checkpoint_id = f"ppo_lag_seed{self.seed}_update{self.agent.update_index:04d}"
        ...
        report["training_algorithm"] = "ppo_lagrangian_expected_retention_v1"
    ...
    def train_until(self, stop_after_update: int) -> dict[str, Any]:
        ...
        if candidate_interval != validation_interval:
            raise RuntimeError("candidate and validation cadence must match")
        ...
        while self.agent.update_index < target:
            ...
            if self.agent.update_index % candidate_interval == 0:
                ...
                validation_reports.append(
                    str(self.validation_dir / f"{report['checkpoint_id']}.json")
                )
        return {
            ...
            "dual_by_eta": {f"{eta:.2f}": value for eta, value in self.agent.dual_by_eta.items()},
        }


def main() -> None:
    parser = argparse.ArgumentParser(description="Train AWM PPO-Lagrangian v1 on real DSSAT")
...
__all__ = ["PPOLagrangianTrainer", "hyperparameters_from_protocol"]
